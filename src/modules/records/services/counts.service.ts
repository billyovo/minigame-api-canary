...
export class CountsService {
    ...
    async count(countRecordDto: WinnerRecordFilterDto, countRecordQueryDto: CountFilterQueryDto) : Promise<CountDto>{
        ...
        let query: PipelineStage[] = [
            ...
            { $facet: {
                ...
                rows: [
                    { $skip: (page - 1) * limit },
                    { $limit: limit },
                    { $project: { '_id': 0, 'name': '$_id.name', 'UUID': '$_id.UUID', 'total': '$total' } }
                ]
            }},
